...
describe('Shapefile Conversion', () => {
  ...
  geometries.forEach((geometry) => {
    outputFormats.forEach(({ format, skipFor }) => {
      ...
      it(`should convert ${geometry} Shapefile to ${format}`, async () => {
        // 1. 入力ファイルを読み込む
        const inputFile = await readInputFile(geometry, `${geometry}s.shp.zip`);
        
        // ShapefileをGeoJSONに変換して比較用のデータを取得
        // 直接Shapefileを比較するのは難しいため、GeoJSONに変換して比較
        const inputGeoJSONFile = await readInputFile(geometry, `${geometry}s.geojson`);
